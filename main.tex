...
\color{blue}
This curvature-driven sublimation-deposition process can be described by a two-phases medium evolving under mean curvature flow. This flow is described by the model of \citeA{bretin_phase-field_2019} and is defined by an interface evolution where the normal velocity $v_n$ is proportional to the curvature C, the volume of each phase being conserved. The mean curvature flow is solved with the phase-field method enabling a implicit description of the interface using a function that varies smoothly from a value in ice to another value in air. 
...
\end{equation}
In the following, we will work at constant temperature. We can therefore omit the $d_0 \alpha v_{kin}$ coefficient that acts as a time-scale factor. The canonical form of this equation, solved by the model, is the famous Allen-Cahn equation \cite{bretin_and_denis_discrete-continuous_2015, kaempfer_phase-field_2009}:
...
\color{black}
Considering the dimensional and dimensionless equations, the model physical time can be expressed as \cite{bretin_and_denis_discrete-continuous_2015}: 
\begin{equation}
   t_\varphi = \frac{t_{\text{sim}} dx^2}{\alpha v_{kin} d_0}
\end{equation}
with $t_{\varphi}$ the physical time in s, $t_{\text{sim}}$ the simulated time, $dx$ the image resolution in m. 
\begin{figure}
    \centering
