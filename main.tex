...
When looking at Figure \ref{fig:Tplot}, the series shows an evolution for \textbf{D}, \textbf{k} and \textbf{K}, but also in density, with a maximum of 10 kg m$^{-3}$ increase for the TG2 series and a minimum of 4 kg m$^{-3}$ for the Grad3 series. This density evolution is an artefact caused by misestimation of curvature at the images edges, but which could lead to a relatively realistic increase in density in terms of mass distribution in the sample (Sec. \ref{sec:disc}). The estimates \textbf{k}, \textbf{D} and \textbf{K} are shown along different parameterizations taken as reference and presented below:\\
\begin{subequations}
\begin{align}
\mathbf{k}_{\mathrm{Calonne\ 2012}}=2.5 \times 10^{-6} \rho_{\mathrm{s}}^{2}-1.23 \times 10^{-4} \rho_{\mathrm{s}}+0.024\end{align}
\begin{align}
\mathbf{D}_{\mathrm{SC}} = D_v(3\phi - 1)/2
\end{align}
\begin{align}
\mathbf{K}_{\mathrm{Calonne\ 2012}}=(3.0 \pm 0.3)\ r_{\mathrm{es}}^{2} \exp \left((-0.0130 \pm 0.0003) \rho_{\mathrm{s}}\right)
\end{align}
\end{subequations}
with $D_v$ the water vapor diffusion coefficient in air.\\

The trends of \textbf{k}, \textbf{D} and \textbf{K} as a function of the porosity represented by the arrows and by the relative changes $\tau_{x y}$ and $\tau_{x y}$ seems to follow the different parameterizations chosen for each estimate except for Grad3 and TG2 effective diffusion coefficients. This shows that the predicted ETM is in overall good agreement with the expected evolution of those macroscopic properties. For Grad3 and TG2 evolution in \textbf{D}, those trends can be explained by the influence of the microstructure. As shown in \citeA{calonne_macroscopic_2014}, the effective coefficient of diffusion seems to increase when evolving from depth hoar to faceted crystals and to rounded grains for a sample of equal density. This effect could overcome the opposing effect of the density evolution. The influence of the microstructure is also present for the permeability as shown in \citeA{calonne_3D_2012} with permeability increasing from rounded grains to faceted crystals and to depth hoar. This effect is in the same direction as the one of the density for the permeability and can be seen as an amplification of the trend. For the thermal conductivity, the microstructure impact is not that distinct as seen in \cite{calonne_numerical_2011}.
