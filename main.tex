...
\begin{figure}
    ...
    \caption{Curvature map of a three-dimensional representation of a tomographic snow sample evolution from \protect\citeA{hagenmuller_motion_2019} under the equi-temperature metamorphism model Snow-3D after 0, 8 and 16 days at -2$^\circ$C. Concave surfaces are shown in green and convex surfaces in red \protect\cite{ogawa2006representation}. }
    \label{fig:eboni_sous_volume}
\end{figure}

% APPLICATION A LA NEIGE (EQUATIONS ETC)
The model of \cite{bretin_phase-field_2019} is applied to the physics of snow equi-temperature metamorphism via the sublimation and deposition process at the ice/air interface due to water vapor density variations driven by curvature. Vapor transport in the pore space by diffusion or convection is not simulated and is assumed to be instantaneous, so the evolution is limited by the kinetics at the interface. Finally, the model does not include any mechanics and the settling of the ice grains is thus not described here.
...
%
At the interface $\Gamma$, the saturation density depends on the surfaces curvature and on the temperature, which is considered isotropic and constant. It is described by the Gibbs-Thomson relation:\\
...
%\end{equation}
The effect of the curvature-driven evolution is illustrated in Figure \ref{fig:eboni_sous_volume} where ice sublimates on high curvature surfaces and water vapor deposits on low curvature surfaces, leading to rounding, coarsening and sintering the ice structure.\\
...
%\color{blue}
\citeA{bretin_phase-field_2019} model simulates a two-phases medium evolving under mean curvature flow. This flow  is defined by an interface evolution where the normal velocity $v_n$ is proportional to the curvature C, the volume of each phase being conserved. The morphological transformation induced by the mean curvature flow can be interpreted as "smoothing" surfaces and is equivalent to sublimation on high curvature surfaces and deposition on negative curvature surfaces. It is applied here on the curvature-driven sublimation-deposition process description. The mean curvature flow is solved with the phase-field method enabling a implicit description of the interface using a function that varies smoothly from a value in ice to another value in air. The phase-field equation can be expressed as \cite{bretin_and_denis_discrete-continuous_2015}:\\
...
\end{equation}
with $t_{\varphi}$ the physical time in s and $t_{\text{sim}}$ the simulated time. Finding a value of the condensation coefficient $\alpha$ is thus equivalent to scale the simulation steps to real time steps.
