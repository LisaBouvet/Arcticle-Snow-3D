...
Using the Hertz-Knudsen equation vapor density can be linked to the growth speed of the interface $\Gamma$:\\
\begin{subequations}
\begin{align}
...
\noindent This equation relates the curvature to the interface velocity normal to the interface oriented toward the air $v_n$ and defines the kinetic velocity $v_{\mathrm{kin}}$. $\alpha$ is the condensation coefficient, $m$ is the mass of a water molecule, $k$ is the Boltzmann constant and $\rho_i$ is the ice density.
 $\rho_{vs}(T, C)$ is the saturation vapor density at an interface of mean curvature $C$ (m$^{-1}$) and of temperature $T$ (K). The saturation vapor density $\rho_{vs}(T, C=0)$ in air above a flat surface has been largely studied, and can be determined using one of the existing parameterizations. We use the formulation of \citeA{gofflow} which is appropriated for our range of temperature and can be found in \citeA{murphy2005review}.\\
