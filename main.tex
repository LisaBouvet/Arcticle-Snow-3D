...
\color{blue}
The model of \citeA{bretin_phase-field_2019} simulates a two-phases medium evolving under mean curvature flow, which is defined by an interface evolution where the normal velocity $v_n$ is proportional to the curvature C, the volume of each phase being conserved. The mean curvature flow is solved with the phase-field method enabling a implicit description of the interface using a function that varies smoothly from a value in ice to another value in air. This is done by solving the famous Allen-Cahn equation \cite{bretin_and_denis_discrete-continuous_2015, kaempfer_phase-field_2009}:
...
u(x,t) := \frac{1}{2}(1-tanh(\frac{s}{2}))(\frac{d(x,t)}{\varepsilon})    
\end{equation}
...
% Generally $(2 \gamma \Omega C / R T) \ll 1$, we can thus write:
% \begin{equation}
% \Delta \rho_v=\rho_v - \rho_v^{\mathrm{ref}} \simeq \rho_v^{\mathrm{ref}}(T)\left(\frac{2 \gamma \Omega C}{R T}\right)\label{eq:kelv_lin} 
% \end{equation}
% This equation gives us access to t distribution within pores depending on the surrounding interface curvatures. 
...
This equation defines the kinetic velocity $v_{\mathrm{kin}}$. $\alpha$ is the condensation coefficient, $m$ is the mass of a water molecule, $\rho_i$ is the ice density and $\sigma$ is the local supersaturation. 
